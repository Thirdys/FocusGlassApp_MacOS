...
- Keep the active session path short: pick mode, set intention, choose project,
  add task, start.
...
- Fullscreen focus must remove nonessential chrome. Running sessions hide
  controls until hover but always keep Escape/close available. Strict blocking
  only runs in this mode.
...
- Schema v4 preserves legacy `state.json` migration and blocks autosave if
...
- The app follows macOS light/dark by default while preserving the selected
  accent theme; the user can pin light or dark mode.
...
- App and site distraction rules for fullscreen strict focus.

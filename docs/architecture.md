...
- `FocusGlassWorkspaceState` writes `workspace.json`: `intention`,
  `activeProjectID`, legacy readable `activeProject`, `projects`, `tasks`,
  `distractionRules`, and `recentSessions`.
- `FocusGlassSettingsState` writes `settings.json`: `selectedThemeID`,
  `themeProfiles`, `appearanceMode`, `selectedPresetID`, `timerPresets`,
  `language`, `strictModeEnabled`, and `hasSeenPermissionsOnboarding`.
...
- `language`, `strictModeEnabled`, `hasSeenPermissionsOnboarding`: app-level
  preferences and first-launch permission onboarding state.
- `intention`, `activeProjectID`, `activeProject`: current focus context.
  `activeProject` is retained only as a legacy/readable string.
...
- `tasks`: `FocusTask` values keyed to projects by optional `projectID`.
  `projectName` is decoded only for legacy migration and is not the source of
  truth after v3.
- `recentSessions`: `FocusSessionRecord` values keyed to projects only by
  optional `projectID`, with `projectName` retained only as a legacy/readable
  string.
...
- `sanitizedStarterState` returns a schema v3 migrated state marker for legacy
...
- Strict rules are evaluated while strict mode is enabled and the timer is
  running. Warn actions leave the target app open, return the user to
  FocusGlass, and show either the fullscreen warning or a normal-mode toast.

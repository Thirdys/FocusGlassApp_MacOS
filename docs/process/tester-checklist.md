...
> Это source snapshot для уже подготовленной ветки `tester/0.0.4`, а не
> чеклист текущей `codex/next`. Следующая tester delivery должна получить новую
> версию и заново сгенерированный cumulative checklist из актуального
> `CHANGELOG.md`, `docs/qa-checklist.md` и `docs/process/tester-rules.md`.
